...
namespace Rooler
{
    ...
    public class ScreenShot : IScreenShot
    {
        ...
        public ScreenShot()
        {
            Capture(ScreenShot.FullScreenBounds);
        }
        ...
        {
            this.bitmap = bitmap;
            pixels = new int[this.bitmap.PixelWidth * this.bitmap.PixelHeight];
        ...
        {
            int newX = x.Clamp(0, bounds.Width - 1);
            int newY = y.Clamp(0, bounds.Height - 1);

            //Debug.Assert(newX == x && newY == y);

            return pixels[newY * bounds.Width + newX];
        }
