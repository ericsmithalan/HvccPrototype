...
namespace Rooler
{
    public class NativeMethods
    {
        ...
        public class DC : IDisposable
        {
            ...
            {
                return NativeMethods.BitBlt(IntPtr, x, y, nWidth, nHeight, hSrcDC.IntPtr, xSrc, ySrc, dwRop);
            }

            public GdiObject SelectObject(GdiObject hgdiobj)
            {
                return new GdiObject(NativeMethods.SelectObject(IntPtr, hgdiobj.IntPtr));
            }

            public DC CreateCompatibleDC()
            {
                return new DC(NativeMethods.CreateCompatibleDC(IntPtr));
            }

            public GdiObject CreateCompatibleBitmap(int width, int height)
            {
                return new GdiObject(NativeMethods.CreateCompatibleBitmap(IntPtr, width, height));
            }
        }
